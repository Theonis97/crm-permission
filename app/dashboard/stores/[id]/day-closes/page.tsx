...
interface StoreDayClosesPageProps {
  params: Promise<{
    id: string
  }>
}

export default function StoreDayClosesPage({ params }: StoreDayClosesPageProps) {
  ...
  useEffect(() => {
    ...
    loadDayCloses()
    loadUsers()
  }, [storeId, currentPage, selectedUser, startDate, endDate])
  ...
  return (
    <>
      <StorePageHeader
        ...
        description={`Historique des clôtures de caisse${store ? ` - ${store.name}` : ''}`}
        icon={Calendar}
      />
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedDayClose && (
                  <>
                    {store?.name} • Clôturé par {selectedDayClose.user.name}
                  </>
                )}
              </DialogDescription>
            </DialogHeader>
