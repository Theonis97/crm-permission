...
export function CreateInvoiceSheet({ open, onOpenChange, onSuccess }: CreateInvoiceSheetProps) {
  ...
  const dueDateFormatted = dueDate ? new Date(dueDate).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) : ''
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-4xl overflow-y-auto p-0 [&>*]:pointer-events-auto">
        <div className="bg-white min-h-full relative">
          {/* En-tête du document - Style facture compact */}
          <div className="bg-gradient-to-r from-green-600 to-green-500 text-white p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-4">
                <div className="z-50">
                  <Label className="text-xs text-green-100 uppercase block mb-1">Client</Label>
                  <Select value={selectedContactId} onValueChange={setSelectedContactId} required>
                    <SelectTrigger className="bg-white text-gray-900 border-green-400 w-64 cursor-pointer relative z-50">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <Label className="text-xs text-green-100 uppercase block mb-1">N° Facture</Label>
                    <Input
                      value={invoiceNumber}
                      onChange={(e) => setInvoiceNumber(e.target.value)}
                      className="bg-white text-gray-900 font-mono border-green-400 w-40"
                      required
                    />
                  </div>
                  <div>
                    <Label className="text-xs text-green-100 uppercase block mb-1">Conditions</Label>
                    <Select value={paymentTerms} onValueChange={updateDueDateFromTerms}>
                      <SelectTrigger className="bg-white text-gray-900 border-green-400 w-32">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="0">Immédiat</SelectItem>
                        <SelectItem value="15">15j</SelectItem>
                        <SelectItem value="30">30j</SelectItem>
                        <SelectItem value="45">45j</SelectItem>
                        <SelectItem value="60">60j</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="text-xs text-green-100 uppercase block mb-1">Échéance</Label>
                    <Input
                      type="date"
                      value={dueDate}
                      onChange={(e) => setDueDate(e.target.value)}
                      className="bg-white text-gray-900 border-green-400 w-36"
                      required
                    />
                  </div>
                </div>
            ...
            {selectedContact && (
              <div className="flex items-center space-x-4 text-xs text-green-100 border-t border-green-400/30 pt-3">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8 pb-24">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-semibold text-gray-500 uppercase">Détail des prestations</h3>
            ...
            {/* Totaux - Style facture professionnelle avec TVA */}
            {items.length > 0 && (
              <div className="flex justify-end">
                <div className="w-96">
                  ...
                  <div className="flex justify-between items-center bg-green-600 text-white p-4 rounded">
                    ...
                    <span className="text-2xl font-bold">{totalTTC.toLocaleString()} XAF</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-2 text-right">
                    Montant à régler avant le {dueDateFormatted}
                  </p>
                </div>
              </div>
            )}
            ...
            <div>
              <Label className="text-sm font-semibold text-gray-500 uppercase">Conditions et modalités de règlement</Label>
              <Textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Modalités de paiement, coordonnées bancaires, conditions générales..."
                rows={4}
                className="mt-2"
              />
