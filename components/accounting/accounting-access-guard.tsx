...
interface AccountingAccessContextType {
  isVerified: boolean
  resetAccess: () => void
}

const AccountingAccessContext = createContext<AccountingAccessContextType>({
  isVerified: false,
  resetAccess: () => {},
})

export const useAccountingAccess = () => useContext(AccountingAccessContext)

interface AccountingAccessGuardProps {
  children: React.ReactNode
}

export function AccountingAccessGuard({ children }: AccountingAccessGuardProps) {
  ...
  // Réinitialiser l'accès (appelé quand l'utilisateur quitte la section)
  ...
  // Countdown pour le temps restant
  useEffect(() => {
    if (!expiresAt) return
  ...
  const requestCode = async () => {
    ...
    try {
      const response = await fetch("/api/accounting/access/request-code", {
        method: "POST",
      })
  ...
  const verifyCode = async () => {
    ...
    try {
      const response = await fetch("/api/accounting/access/verify-code", {
        method: "POST",
  ...
  if (isVerified) {
    return (
      <AccountingAccessContext.Provider value={{ isVerified, resetAccess }}>
        {children}
      </AccountingAccessContext.Provider>
    )
  }
        ...
        <CardContent className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500">
              L'accès à la comptabilité nécessite une double authentification.
            </p>
          </div>
